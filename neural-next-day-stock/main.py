...
for i in range(14):
    new_lookup_step = i + 1
    # Now test the model
    data = load_data(tickers[0], N_STEPS, lookup_step=new_lookup_step, test_size=TEST_SIZE,
                     feature_columns=FEATURE_COLUMNS, shuffle=False)

    # predict the future price
    future_price = predict(model, data, N_STEPS)
    print(f"Price after {new_lookup_step} days is {future_price:.2f}$")


    # Print accuracy
    print(str(new_lookup_step) + ":", "Accuracy Score:", get_accuracy(model, data, new_lookup_step))
